...
function prepareTableWithData(json, callback) {
    ...
    for (var i = 0; i < dataToUse.series.length; i++) {
        ...
        for (var j = 1; j < dataToUse.series[i].data.length; j++) {
            targetRow.append('<td class="dataCell"><div class="row"><div class="col-xs-8 text-left"><span class="gbar" style="width: ' +
                (dataToUse.series[i].data[j].y) + '"></span></div><div class="col-xs-4">' + (dataToUse.series[i].data[j].y) + '</div></div></td>');
        }
    }
